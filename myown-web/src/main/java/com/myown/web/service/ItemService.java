...
public class ItemService {
    ...
    public static final String REDIS_KEY = "MYOWN_WEB_ITEM_DETAIL_";
    ...
    public Item queryById(Long itemId) {
        ...
        String url = MYOWN_MANAGE_URL+"/rest/api/item/"+itemId;
        try {
    ...
    public ItemDesc queryDescById(Long itemId) {
        String url = MYOWN_MANAGE_URL+"/rest/api/item/desc/"+itemId;
        try {
            String jsonData = this.apiService.doGet(url);
            if(StringUtils.isEmpty(jsonData)){
                return null;
    ...
    public String queryItemParamItemByItemId(Long itemId) {
        String url = MYOWN_MANAGE_URL+"/rest/api/item/param/item/"+itemId;
        try {
            String jsonData = this.apiService.doGet(url);
            if(StringUtils.isEmpty(jsonData)){
                return null;
            }
            //将json数据反序列为ItemParamItem对象
